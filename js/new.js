...
const displayNewsCard = (newsDisplay) => {
    ...
    newsDisplay.forEach(newsDetails => {
        ...
        <div class="card">
            ...
            <div class="card-body">
            <h1 class="card-title sm:text-1xl">News Title : ${newsDetails.title.slice(0,55)} </h1>
            <p class="card-text sm:text-1xl">${newsDetails.details.slice(0,250)}</p>
            
            <div class="d-flex justify-content-between align-middle mt-2">
            <h1 class="card-title p-1 sm:text-sm">Author: ${newsDetails.author.name}, </h1> <p class="p-1 ps-5 mt-1">View: ${newsDetails.total_view}k</p>
